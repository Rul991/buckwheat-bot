...
export default class ChatIdRepository<T extends typeof Model, K extends { id: number, chatId: number }> extends BaseRepository<K, T> {
    ...
    async updateMany(chatId: number, data: UpdateQuery<K>): Promise<UpdateWriteOpResult> {
        return await super.updateMany({ chatId }, data)
    }

    async getOrCreate(chatId: number, id: number, data: K): Promise<K> {
        return await super.getOrCreate({ chatId, id }, data)
    }
}